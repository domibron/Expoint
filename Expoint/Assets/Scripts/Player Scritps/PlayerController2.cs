...
public class PlayerController2 : MonoBehaviourPunCallbacks, IDamageable
{
    ...
    void Awake()
    {
        ...
        playerManager2 = PhotonView.Find((int)PV.InstantiationData[0]).GetComponent<PlayerManager2>();
    }
    ...
    void Start()
    {
        if (PV.IsMine)
        {
            ...
            // playerObject.layer = 7;
            // foreach (Transform child in transform)
            // {
            //     child.gameObject.layer = 7;
            // }

            Transform[] childs = playerObject.transform.GetComponentsInChildren<Transform>();
    ...
    void Move()
    {
        ...
        float animation_horizontal = 0f;
        float animation_vertical = 0f;

        if (grounded)
        {
            animation_horizontal = moveDir.x;
            animation_vertical = moveDir.z;
        }

        animator.SetFloat("Horizontal", animation_horizontal, 0.1f, Time.deltaTime);
        animator.SetFloat("Vertical", animation_vertical, 0.1f, Time.deltaTime);
    ...
    void RPC_TakeDamage(float damage, PhotonMessageInfo info)
    {
        ...
        {
            Die();
            PlayerManager2.Find(info.Sender).GetKill();
        }
    }
